...
function closeMySpecies() {
    console.log("Closing chooseMySpecies");
    $('#SpeciesListDialog').dialog( "close" );
}
function chooseMySpecies() {
    console.log("Building Personal Species select");
    var pSelList = '<form><fieldset><legend>Select Species</legend><div id="checkboxes">';
    for(var i=0;i<DBFuncs.SpeciesList.length;i++){
        ...
        pSelList+='<input type="checkbox" name="speciesSel" value="'+ lStr + '" />' + lStr + ' <br />';
    }
    
    var spl = "#SpeciesListDialog";

    pSelList += '</div><input type="button" value="Save" onclick="closeMySpecies()"/></fieldset></form>';
    $('#SpeciesListSel').html(pSelList).trigger( "create" );
    //$('#SpeciesListDialog').selectmenu();
    var ret = $('#SpeciesListDialog').dialog({});
    $.mobile.changePage('#SpeciesListDialog',  { role: "dialog" });
